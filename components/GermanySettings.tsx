...
export default function GermanySettings({ userId, diaryStartMonth, expenseCategories, onSaveState }: GermanySettingsProps) {
  ...
  async function load() {
    ...
    const { expenses: loadedExpenses, remainingRows } = splitPlanExpenses(plan, diaryStartMonth, fallbackGroup);
  ...
  async function saveNow(next = latestExpensesRef.current, announce = true) {
    ...
    if (announce) onSaveState({ status: "saving", message: "Сохраняются расходы Германии…" });

    const { data: current, error: loadPlanError } = await supabase.from("relocation_plans").select("data").eq("user_id", userId).maybeSingle();
    if (loadPlanError) {
      onSaveState({ status: "error", message: loadPlanError.message });
      return;
    }
    ...
    const remainingRows = rows.filter((row: any) => !(row?.country === "DE" && row?.kind === "expense" && row?.frequency !== "once"));
    const mergedPlan = { ...currentPlan, rows: remainingRows, germanyExpenses: next };
    ...
    if (error) {
      onSaveState({ status: "error", message: error.message });
      return;
    }
  ...
  function updateExpense(id: string, patch: Partial<GermanyExpense>) {
    setExpensesAndSave(expenses.map((row) => row.id === id ? { ...row, ...patch } : row));
  }

  function deleteExpense(id: string) {
    setExpensesAndSave(expenses.filter((row) => row.id !== id));
  }
            ...
            <tbody>
              {visible.map((row) => (
                <tr key={row.id} className={row.active ? "" : "inactive"}>
                  ...
                  <td><input type="number" value={row.amount} onChange={(e) => updateExpense(row.id, { amount: Number(e.target.value || 0) })} /></td>
                  ...
                  <td><MonthPicker value={row.startMonth} onChange={(value) => updateExpense(row.id, { startMonth: value || diaryStartMonth })} /></td>
