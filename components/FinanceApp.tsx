...
function normalizeMonth(v: string | null | undefined) {
  const text = typeof v === "string" ? v.slice(0, 7) : "";
...
function cleanMonth(value: unknown) {
  return normalizeMonth(typeof value === "string" ? value : null);
}
...
function uniqueNames(values: unknown[], fallback: string[]) {
...
export default function FinanceApp({ userId }: { userId: string }) {
  ...
  useEffect(() => () => {
    void flushPendingDatabaseSaves();
  }, [userId]);
  ...
  async function loadAll() {
    ...
    try {
      ...
      setPayments((payData || []).map((row: any) => {
        const totalMonths = nonNegative(row.total_months);
        return {
          ...row,
      ...
      setViewMonthState((current) => safeViewMonth(current, loadedDiaryStart));
    } catch (error: any) {
  ...
  async function moveOperation(op: Operation, dir: -1 | 1) {
    ...
    const currentResult = await supabase.from("operations").update({ sort_order: targetOrder }).eq("user_id", userId).eq("id", current.id);
    if (currentResult.error) {
      flash(currentResult.error.message);
      return;
    }

    const targetResult = await supabase.from("operations").update({ sort_order: currentOrder }).eq("user_id", userId).eq("id", target.id);
    if (targetResult.error) {
      await supabase.from("operations").update({ sort_order: currentOrder }).eq("user_id", userId).eq("id", current.id);
      flash(targetResult.error.message);
      return;
    }
  ...
  const pagedRows = displayPages[page - 1] || [];
  ...
  async function addCategory(kind: Kind) {
    ...
    const baseName = kind === "expense" ? "Новая статья расходов" : "Новая статья доходов";
    const existingNames = new Set(list.map((item) => item.name.trim().toLocaleLowerCase("ru-RU")));
    let name = baseName;
    let suffix = 2;
    while (existingNames.has(name.toLocaleLowerCase("ru-RU"))) name = `${baseName} ${suffix++}`;
  ...
  async function replaceRelocationGroup(kind: Kind, oldName: string, newName: string) {
    if (!oldName || oldName === newName) return null;
    const { data, error: loadError } = await supabase.from("relocation_plans").select("data").eq("user_id", userId).maybeSingle();
    if (loadError) return loadError;
    if (!data?.data) return null;

    const plan = data.data;
    const rows = Array.isArray(plan.rows)
      ? plan.rows.map((row: any) => row?.kind === kind && row?.group === oldName ? { ...row, group: newName } : row)
      : [];
    const germanyExpenses = kind === "expense" && Array.isArray(plan.germanyExpenses)
      ? plan.germanyExpenses.map((row: any) => row?.group === oldName ? { ...row, group: newName } : row)
      : (Array.isArray(plan.germanyExpenses) ? plan.germanyExpenses : []);

    const { error } = await supabase.from("relocation_plans").upsert({
      user_id: userId,
      data: { ...plan, rows, germanyExpenses },
      updated_at: new Date().toISOString()
    });
    if (!error) {
      window.dispatchEvent(new CustomEvent("relocation-plan-updated", {
        detail: { groupRename: { kind, oldName, newName } }
      }));
    }
    return error;
  }
  ...
  async function saveCategory(kind: Kind, id: string, name: string) {
    ...
    const sourceList = kind === "expense" ? expenseCategories : incomeCategories;
    if (sourceList.some((item) => item.id !== id && item.name.trim().toLocaleLowerCase("ru-RU") === cleanName.toLocaleLowerCase("ru-RU"))) {
      flash("Статья с таким названием уже существует");
      return;
    }
    const oldName = sourceList.find((item) => item.id === id)?.name || "";
    beginSave();
    const table = kind === "expense" ? "expense_categories" : "income_categories";
    ...
    const relocationError = await replaceRelocationGroup(kind, oldName, cleanName);
    if (relocationError) {
      failSave(`Статья сохранена, но план Германии не обновлён: ${relocationError.message}`);
      return;
    }
    finishSave();
  }
  ...
  async function deleteCategory(kind: Kind, id: string) {
    ...
    const fallbackCategory = list.find((item) => item.id !== id)!;
    const table = kind === "expense" ? "expense_categories" : "income_categories";
    ...
    const relocationError = await replaceRelocationGroup(kind, category.name, fallbackCategory.name);
    if (relocationError) {
      failSave(`Статья удалена, но план Германии не обновлён: ${relocationError.message}`);
      return;
    }
  ...
  function updatePayment(id: string, patch: Partial<RecurringPayment>) {
    ...
    const nextTotal = patch.total_months !== undefined
      ? nonNegative(patch.total_months)
      : nonNegative(current.total_months);
    const nextPaid = patch.paid_months !== undefined
      ? nonNegative(patch.paid_months)
      : nonNegative(current.paid_months);
  ...
  function updateIncome(id: string, patch: Partial<RecurringIncome>) {
    const current = incomes.find((income) => income.id === id);
    if (!current) return;
  ...
  async function exportData() {
    const { data: relocationData, error } = await supabase.from("relocation_plans").select("data").eq("user_id", userId).maybeSingle();
    if (error) {
      flash(`Не удалось выгрузить план Германии: ${error.message}`);
      return;
    }
    ...
    const payload = {
      ...
      exclusions,
      collapsedGroups,
      relocationPlan: relocationData?.data || null,
    ...
    anchor.href = url;
    anchor.download = `finance-diary-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  async function importLegacy(file: File | null) {
    ...
    window.dispatchEvent(new CustomEvent("relocation-plan-before-import"));
    await waitForRelocationSaves();
    setLoading(true);
    ...
    try {
      ...
      const groups = uniqueNames(
        isSupabaseExport
          ? (legacy.expenseCategories || []).map((c: any) => c.name)
          : (Array.isArray(legacy.groups) ? legacy.groups : []),
        defaultExpenseCategories
      );
      const incomeTypes = uniqueNames(
        isSupabaseExport
          ? (legacy.incomeCategories || []).map((c: any) => c.name)
          : (Array.isArray(legacy.incomeTypes) ? legacy.incomeTypes : []),
        defaultIncomeCategories
      );

      const { data: expCats, error: expErr } = await supabase
        .from("expense_categories")
        .insert(groups.map((name: string, i: number) => ({ user_id: userId, name, sort_order: (i + 1) * 10 })))
        .select("*");
      if (expErr) throw expErr;

      const { data: incCats, error: incErr } = await supabase
        .from("income_categories")
        .insert(incomeTypes.map((name: string, i: number) => ({ user_id: userId, name, sort_order: (i + 1) * 10 })))
        .select("*");
      ...
      const relocationPlan = legacy.relocationPlan || legacy.relocation_plan || null;
      if (relocationPlan && typeof relocationPlan === "object") {
        const { error } = await supabase.from("relocation_plans").upsert({
          user_id: userId,
          data: relocationPlan,
          updated_at: new Date().toISOString()
        ...
        if (error) throw error;
        window.dispatchEvent(new CustomEvent("relocation-plan-updated", { detail: { replacePlan: relocationPlan } }));
      } else {
        window.dispatchEvent(new CustomEvent("relocation-plan-updated", { detail: { reset: true } }));
      }
              ...
              <table className="matrix">
                <thead>
                  <tr>
                    ...
                    {forecast.map((m) => <th key={`y-${m.month}`} className={forecastCellClass(m.month, "year")}>{m.month.slice(0, 4)}</th>)}
                  </tr>
                  <tr>
                    <th className="corner sub">Месяц</th>
                    {forecast.map((m) => <th key={`m-${m.month}`} className={forecastCellClass(m.month, "month")}>{monthLabel(m.month)}</th>)}
                  </tr>
                </thead>
                <tbody>
